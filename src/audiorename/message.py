...
class Message:
    ...
    def action_two_path(
        self, message: str, source: "AudioFile", target: "AudioFile"
    ) -> None:
        self.status(message, status="progress")
    ...
    def best_format(
        self,
        best: typing.Literal["target", "source"],
        attr: str,
        source: "Meta",
        target: "Meta",
    ) -> None:
    ...
    def status_color(
        self, status: typing.Literal["ok", "error", "progress"]
    ...
    def status(self, text: str, status: typing.Literal["ok", "error", "progress"]):
        if self.color:
